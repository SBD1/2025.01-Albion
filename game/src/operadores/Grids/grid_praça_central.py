...
CHAO_PRACA_RECT = pygame.Rect(607, 991, 639-607, 1018-991)  # Piso de pedra
BLOCANTE1_PRACA_RECT = pygame.Rect(640, 480, 32, 32)  # Muro principal
BLOCANTE2_PRACA_RECT = pygame.Rect(416, 352, 32, 32)  # Estátua
BLOCANTE3_PRACA_RECT = pygame.Rect(288, 352, 32, 32)  # Fonte
BLOCANTE4_PRACA_RECT = pygame.Rect(352, 352, 32, 32)  # Banco
...
# Cores
COLOR_PLAYER = (255, 255, 255)  # Branco para o jogador na praça
COLOR_MONSTER = (255, 0, 0)
COLOR_BLOCK = (100, 100, 100)

def carregar_sprite_chao_praca():
    """Carrega e redimensiona o sprite do chão da praça."""
    sprite_sheet = pygame.image.load(SPRITE_PATH).convert_alpha()
    chao = pygame.Surface((CHAO_PRACA_RECT.width, CHAO_PRACA_RECT.height), pygame.SRCALPHA)
    chao.blit(sprite_sheet, (0, 0), CHAO_PRACA_RECT)
    return pygame.transform.scale(chao, (CELL_SIZE, CELL_SIZE))

def carregar_sprite_blocante1_praca():
    """Carrega e redimensiona o sprite do muro principal."""
    sprite_sheet = pygame.image.load(SPRITE_PATH).convert_alpha()
    blocante = pygame.Surface((BLOCANTE1_PRACA_RECT.width, BLOCANTE1_PRACA_RECT.height), pygame.SRCALPHA)
    blocante.blit(sprite_sheet, (0, 0), BLOCANTE1_PRACA_RECT)
    return pygame.transform.scale(blocante, (CELL_SIZE, CELL_SIZE))

def carregar_sprite_blocante2_praca():
    """Carrega e redimensiona o sprite da estátua."""
    sprite_sheet = pygame.image.load(SPRITE_PATH).convert_alpha()
    blocante = pygame.Surface((BLOCANTE2_PRACA_RECT.width, BLOCANTE2_PRACA_RECT.height), pygame.SRCALPHA)
    blocante.blit(sprite_sheet, (0, 0), BLOCANTE2_PRACA_RECT)
    return pygame.transform.scale(blocante, (CELL_SIZE, CELL_SIZE))

def carregar_sprite_blocante3_praca():
    """Carrega e redimensiona o sprite da fonte."""
    sprite_sheet = pygame.image.load(SPRITE_PATH).convert_alpha()
    blocante = pygame.Surface((BLOCANTE3_PRACA_RECT.width, BLOCANTE3_PRACA_RECT.height), pygame.SRCALPHA)
    blocante.blit(sprite_sheet, (0, 0), BLOCANTE3_PRACA_RECT)
    return pygame.transform.scale(blocante, (CELL_SIZE, CELL_SIZE))

def carregar_sprite_blocante4_praca():
    """Carrega e redimensiona o sprite do banco."""
    sprite_sheet = pygame.image.load(SPRITE_PATH).convert_alpha()
    blocante = pygame.Surface((BLOCANTE4_PRACA_RECT.width, BLOCANTE4_PRACA_RECT.height), pygame.SRCALPHA)
    blocante.blit(sprite_sheet, (0, 0), BLOCANTE4_PRACA_RECT)
    return pygame.transform.scale(blocante, (CELL_SIZE, CELL_SIZE))
...
def desenhar_grid_praca(screen, grid, pos_jogador, sprite_chao, sprite_blocantes, sprite_monstro=None, monstros_sala=None):
    """Desenha o grid da praça com chão, blocantes e jogador."""
        ...
        for j in range(largura):
            if grid[i][j] not in (" ", "@"):
                ...
                if i == 0 or j == 0 or i == altura - 1 or j == largura - 1:
                    sprite_blocante = sprite_blocantes[0]  # Muro principal
                else:
...
def main_grid_pygame_praca(nome_sala="Praça Central", sprites_personagem=None, direcao_atual="baixo"):
    pygame.init()
    ...
    if isinstance(grid_data, list) and isinstance(grid_data[0], str):
        ...
        # Praça Central não tem monstros para limpar, mantém o grid original
        pass
    altura, largura = len(grid), len(grid[0])
    screen = pygame.display.set_mode((largura * CELL_SIZE, altura * CELL_SIZE + 40))
    pygame.display.set_caption("Albion Grid Praça Central - Pygame")
    sprite_chao = carregar_sprite_chao_praca()
    sprite_blocantes = [
        carregar_sprite_blocante1_praca(),
        carregar_sprite_blocante2_praca(),
        carregar_sprite_blocante3_praca(),
        carregar_sprite_blocante4_praca()
    ]
    ...
    while running:
        ...
        screen.fill((0, 0, 0))
        desenhar_grid_praca(screen, grid, pos_jogador, sprite_chao, sprite_blocantes, sprite_monstro, monstros_sala)
...
if __name__ == "__main__":
    ...
    main_grid_pygame_praca(sala_teste)