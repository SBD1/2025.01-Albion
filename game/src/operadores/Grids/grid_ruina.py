import pygame
from .common.grid_engine import GridEngine
from .biomes.ruinas_config import RUINAS_CONFIG

def main_grid_pygame_ruinas(nome_sala="Ruínas Antigas", sprites_personagem=None, direcao_atual="baixo", screen=None):
    """
    Função principal para renderizar e controlar o grid das ruínas.
    Refatorada para usar o GridEngine comum.
    """
    engine = GridEngine(RUINAS_CONFIG)
    return engine.run_grid(nome_sala, sprites_personagem, direcao_atual, screen)
