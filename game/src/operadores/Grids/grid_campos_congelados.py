...
CHAO_NEVE_RECT = pygame.Rect(0, 200, 50, 50)  # Chão nevado
BLOCANTE1_NEVE_RECT = pygame.Rect(317, 0, 457-317, 63)  # Parede de gelo
BLOCANTE2_NEVE_RECT = pygame.Rect(448, 511, 511-448, 639-511)  # Bloco de gelo
BLOCANTE3_NEVE_RECT = pygame.Rect(66, 0, 125-66, 63)  # Estalactite
BLOCANTE4_NEVE_RECT = pygame.Rect(544, 256, 32, 32)  # Monte de neve
...
# Cores
COLOR_PLAYER = (0, 191, 255)  # Azul claro para o jogador no gelo
COLOR_MONSTER = (255, 0, 0)
COLOR_BLOCK = (100, 100, 100)

def carregar_sprite_chao_neve():
    """Carrega e redimensiona o sprite do chão nevado."""
    sprite_sheet = pygame.image.load(SPRITE_PATH).convert_alpha()
    chao = pygame.Surface((CHAO_NEVE_RECT.width, CHAO_NEVE_RECT.height), pygame.SRCALPHA)
    chao.blit(sprite_sheet, (0, 0), CHAO_NEVE_RECT)
    return pygame.transform.scale(chao, (CELL_SIZE, CELL_SIZE))

def carregar_sprite_blocante1_neve():
    """Carrega e redimensiona o sprite da parede de gelo."""
    sprite_sheet = pygame.image.load(SPRITE_PATH).convert_alpha()
    blocante = pygame.Surface((BLOCANTE1_NEVE_RECT.width, BLOCANTE1_NEVE_RECT.height), pygame.SRCALPHA)
    blocante.blit(sprite_sheet, (0, 0), BLOCANTE1_NEVE_RECT)
    return pygame.transform.scale(blocante, (CELL_SIZE, CELL_SIZE))

def carregar_sprite_blocante2_neve():
    """Carrega e redimensiona o sprite do bloco de gelo."""
    sprite_sheet = pygame.image.load(SPRITE_PATH).convert_alpha()
    blocante = pygame.Surface((BLOCANTE2_NEVE_RECT.width, BLOCANTE2_NEVE_RECT.height), pygame.SRCALPHA)
    blocante.blit(sprite_sheet, (0, 0), BLOCANTE2_NEVE_RECT)
    return pygame.transform.scale(blocante, (CELL_SIZE, CELL_SIZE))

def carregar_sprite_blocante3_neve():
    """Carrega e redimensiona o sprite da estalactite."""
    sprite_sheet = pygame.image.load(SPRITE_PATH).convert_alpha()
    blocante = pygame.Surface((BLOCANTE3_NEVE_RECT.width, BLOCANTE3_NEVE_RECT.height), pygame.SRCALPHA)
    blocante.blit(sprite_sheet, (0, 0), BLOCANTE3_NEVE_RECT)
    return pygame.transform.scale(blocante, (CELL_SIZE, CELL_SIZE))

def carregar_sprite_blocante4_neve():
    """Carrega e redimensiona o sprite do monte de neve."""
    sprite_sheet = pygame.image.load(SPRITE_PATH).convert_alpha()
    blocante = pygame.Surface((BLOCANTE4_NEVE_RECT.width, BLOCANTE4_NEVE_RECT.height), pygame.SRCALPHA)
    blocante.blit(sprite_sheet, (0, 0), BLOCANTE4_NEVE_RECT)
    return pygame.transform.scale(blocante, (CELL_SIZE, CELL_SIZE))
...
def desenhar_grid_neve(screen, grid, pos_jogador, sprite_chao, sprite_blocantes, sprite_monstro=None, monstros_sala=None, sprites_personagem=None, direcao_atual="baixo"):
    """Desenha o grid dos campos congelados com chão, blocantes e jogador."""
        ...
        for j in range(largura):
            if grid[i][j] not in (" ", "@"):
                ...
                if i == 0 or j == 0 or i == altura - 1 or j == largura - 1:
                    sprite_blocante = sprite_blocantes[0]  # Parede de gelo
                else:
...
def main_grid_pygame_neve(nome_sala="Campos Congelados", sprites_personagem=None, direcao_atual="baixo"):
    """Função principal para renderizar e controlar o grid dos campos congelados."""
    pygame.init()
    # Inicialização do grid e configuração da janela
    ...
    if isinstance(grid_data, list) and isinstance(grid_data[0], str):
        ...
        # Limpar apenas emoji de monstros específicos dos campos congelados do grid
        for i in range(len(grid)):
            for j in range(len(grid[0])):
                if grid[i][j] == "🐻‍❄️":  # Remove apenas o emoji específico do monstro dos campos congelados
                    grid[i][j] = " "
    altura, largura = len(grid), len(grid[0])
    screen = pygame.display.set_mode((largura * CELL_SIZE, altura * CELL_SIZE + 40))
    pygame.display.set_caption("Albion Grid Campos Congelados - Pygame")
    # Carregamento dos sprites
    sprite_chao = carregar_sprite_chao_neve()
    sprite_blocantes = [
        carregar_sprite_blocante1_neve(),
        carregar_sprite_blocante2_neve(),
        carregar_sprite_blocante3_neve(),
        carregar_sprite_blocante4_neve()
    ]
    sprite_monstro = carregar_sprite_monstro()
    # Configuração inicial do jogador e do jogo
    ...
    while running:
        ...
        # Atualização da tela
        screen.fill((0, 0, 0))
        desenhar_grid_neve(screen, grid, pos_jogador, sprite_chao, sprite_blocantes, sprite_monstro, monstros_sala, sprites_personagem, direcao_atual)
        # Desenho do texto de instrução
...
if __name__ == "__main__":
    ...
    main_grid_pygame_neve(sala_teste)